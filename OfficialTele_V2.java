...
@TeleOp(name="Official TeleOp V2", group="Official")

/*Log for "OfficalTeleOp-Copy"
    FTC - TecTigers 10382
    TeleOP
...
public class OfficialTele_V2 extends OpMode
{
    ...
    public void loop() {
        ...
        //speed of the motors
        if(speedCut){//if the speed cut button was pressed, then the robot drives half the speed
            if(gamepad1.left_stick_y > .05 || gamepad1.left_stick_y < -.05){
                leftFrontDrive.setPower(-gamepad1.left_stick_y * .50);
                leftBackDrive.setPower(-gamepad1.left_stick_y * .50);
            
                rightFrontDrive.setPower(gamepad1.right_stick_y * .50); 
                rightBackDrive.setPower(gamepad1.right_stick_y * .50);
            }else{//if the speed cut button isn't pressed or normal speed is pressed, then run normally
        ...
        if(gamepad1.x){
            latchingServo.setPosition(0.0);//originally 1
        }else if(gamepad1.y){
            latchingServo.setPosition(0.25);//originally 0
        }
